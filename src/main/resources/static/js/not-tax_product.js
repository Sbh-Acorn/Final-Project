...
// 조건에 따라 leftValue 설정
let leftValue = sortedMinPrice === 0
    ? 0
    : (sortedMinPrice / parseFloat($range.max)) * 100;

let rightValue = sortedMaxPrice === parseFloat($range.max)
    ? 100
    : (sortedMaxPrice / parseFloat($range.max)) * 100; // 최대값은 기본적으로 100%
